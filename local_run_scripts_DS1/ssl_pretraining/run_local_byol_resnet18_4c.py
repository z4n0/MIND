...
DATA_ROOT = Path(os.environ.get("DATA_ROOT", DEFAULT_DATA_ROOT)).expanduser().resolve()
...
def save_weights(weights_path: str, learner, simsiam_flag, class_names, loader_type = "patch") -> Path:
    # Create dynamic filename based on configuration
    base_name = Path(weights_path).stem  # Gets filename without extension
    base_dir = Path(weights_path).parent # Gets the directory of the weights path
    extension = Path(weights_path).suffix # Gets the file extension

    # Get configuration details for filename
    method = "simsiam" if simsiam_flag else "byol"
    # loader_type = loader_type  # "patch" or "full_image"
...
def main() -> None:
    ...
    loader_type = cfg.data_loading["loader_type"]
    ...
    # Loader selection
    if loader_type == "patch":
        train_loader = train_loader_patch_ds
    elif loader_type == "full_image":
    ...
    w_path = save_weights(cfg.output["weights_path"], learner=learner, simsiam_flag=simsiam_flag, class_names=class_names, loader_type=loader_type)
